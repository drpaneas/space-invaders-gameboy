...
uint8_t GetAvailableBulletSprite(void)
{
...
void UpdateBullets(void)
{
    ...
    for (uint8_t i = 2; i < 8; i++)
    {
        ...
        if (shadow_OAM[i].tile != 0)
        {
            ...
            if (shadow_OAM[i].tile == ALIEN_SPRITE_START)
                shadow_OAM[i].tile = BULLETC_SPRITE_START;
            if (shadow_OAM[i].tile == BULLETC_SPRITE_START)
                shadow_OAM[i].tile = BULLETB_SPRITE_START;
            else if (shadow_OAM[i].tile == BULLETB_SPRITE_START)
                shadow_OAM[i].tile = BULLETA_SPRITE_START;
            ...
            if (shadow_OAM[i].y > 176 || shadow_OAM[i].y > 250)
                shadow_OAM[i].tile = 0;
            ...
            else
            {
                ...
                if (i > 2)
                {
                    if (shadow_OAM[i].x - 4 < player.x - 8)
                        continue;
                    if (shadow_OAM[i].x - 4 > player.x + 8)
                        continue;
                    if (shadow_OAM[i].y - 12 < player.y - 4)
                        continue;
                    if (shadow_OAM[i].y - 12 > player.y + 4)
                        continue;
