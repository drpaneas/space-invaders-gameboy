...
uint8_t SlideInvader(uint8_t i)
{
    ...
    if (invaders[i].column == 0 && slideDir != 1)
        return 0;
    if (invaders[i].column == 19 && slideDir == 1)
        return 0;
    ...
    if (invaders[i].slide > 7 || invaders[i].slide < -7)
    {
        ...
        if (i > 0)
        {
            if (invaders[i - 1].column > invaders[i].column)
...
void ShiftAllInvadersDown(void)
{
...
uint8_t InvadersShouldMove(void)
{
    if (topRow > 12)
        return 1;
