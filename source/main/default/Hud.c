...
void DrawText(uint8_t x, uint8_t y, unsigned char *text)
{
    ...
    while (text[i] != '\0')
    {
        ...
        if (text[i] >= 'A' && text[i] <= 'Z')
            set_vram_byte(vramAddr++, SPACEINVADERSFONT_TILES_START + (text[i] - 'A'));
        else if (text[i] >= 'a' && text[i] <= 'z')
            set_vram_byte(vramAddr++, SPACEINVADERSFONT_TILES_START + (text[i] - 'a'));
        else if (text[i] >= '0' && text[i] <= '9')
            set_vram_byte(vramAddr++, SPACEINVADERSFONT_TILES_START + 26 + (text[i] - '0'));
        ...
        else
        {
            ...
            {
            case '.':
                set_vram_byte(vramAddr++, SPACEINVADERSFONT_TILES_START + 36);
                break;
            case '-':
                set_vram_byte(vramAddr++, SPACEINVADERSFONT_TILES_START + 37);
                break;
            case '=':
                set_vram_byte(vramAddr++, SPACEINVADERSFONT_TILES_START + 38);
                break;
            case '*':
                set_vram_byte(vramAddr++, SPACEINVADERSFONT_TILES_START + 39);
                break;
            case '?':
                set_vram_byte(vramAddr++, SPACEINVADERSFONT_TILES_START + 40);
                break;
            case '!':
                set_vram_byte(vramAddr++, SPACEINVADERSFONT_TILES_START + 41);
                break;
            case ' ':
...
void DrawNumber(uint8_t x, uint8_t y, uint16_t number, uint8_t digits)
{
    ...
    unsigned char buffer[9] = "00000000"; // 8 digits max + null terminator
